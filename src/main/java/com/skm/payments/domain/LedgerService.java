...
/**
 * The ledger primitive: post a balanced, immutable double-entry transaction and maintain account
 * balances safely under concurrency.
 *
 * <p>Balance convention: a CREDIT increases an account's balance, a DEBIT decreases it. Balance
 * rows are locked with {@code SELECT ... FOR UPDATE} in ascending account-id order (deterministic
 * lock ordering avoids deadlocks), the signed delta is applied, and the non-negative invariant is
 * enforced before the postings are written — all in one transaction. The database also enforces the
 * net-zero invariant via a deferred trigger.
 */
@Service
public class LedgerService {
  ...
  /**
   * Post a balanced transaction; returns the new transaction id. Runs in one DB transaction. Throws
   * {@link InsufficientFundsException} if any debit would make a balance negative (the whole
  ...
  public UUID post(TransactionType type, UUID referenceId, List<Posting> postings) {
    ...
    List<Posting> lockOrdered =
        postings.stream().sorted(Comparator.comparing(Posting::accountId)).toList();

    for (Posting p : lockOrdered) {
      AccountBalance balance =
          balances
              .findByIdForUpdate(p.accountId())
              .orElseThrow(
                  () -> new IllegalStateException("no balance row for account " + p.accountId()));
  ...
  /**
   * The double-entry invariant: at least 2 postings, each with a positive amount, whose signed sum
   * is zero (DEBIT = +amount, CREDIT = -amount, so total debits = total credits).
   */
