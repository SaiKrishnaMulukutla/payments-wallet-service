 ...
 *
 * <ul>
 *   <li><b>Default</b> — spins a singleton Postgres via Testcontainers (needs a reachable
 *       Docker daemon; the normal way to run on a developer host).</li>
 *   <li><b>External DB</b> — if {@code EXTERNAL_DB_URL} is set, connects to that Postgres
 *       instead of starting one. Used when the build itself runs inside a container on the
 *       same Docker network as a sibling Postgres (e.g. on Rancher Desktop, where mounting
 *       the Docker socket into the build container isn't reliable).</li>
 * </ul>
 */
@SpringBootTest
